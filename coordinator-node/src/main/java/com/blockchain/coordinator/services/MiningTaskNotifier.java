...
public class MiningTaskNotifier {
    ...
    public void notifyNewMiningTask(Block blockCandidate, String hashChallenge, int retries) {
        ...
        logger.info("MiningTaskNotifier: Tarea de minería publicada. evento NEW_CANDIDATE_BLOCK  publicado");
    }

    public void notifySolvedCandidateBlock(String preliminaryHash, String minerId) {
        MiningTaskStatus solvedTaskStatus = new MiningTaskStatus(ExchangeEvent.RESOLVED_CANDIDATE_BLOCK, preliminaryHash, minerId);
        MessageProperties properties = new MessageProperties();
        properties.setDeliveryMode(MessageProperties.DEFAULT_DELIVERY_MODE.PERSISTENT);
        Message message = jsonMessageConverter.toMessage(solvedTaskStatus, properties);
        rabbitTemplate.send(RabbitMQConfig.BLOCKCHAIN_EXCHANGE, "", message);
        logger.info("MiningTaskNotifier: Notificado que el bloque: " + preliminaryHash + " fue resuelto por " + minerId + ". Evento RESOLVED_CANDIDATE_BLOCK publicado.");
    }

    public void notifyMiningTaskDropped(String preliminaryHash) {
        MiningTaskStatus droppedTaskStatus = new MiningTaskStatus(ExchangeEvent.CANDIDATE_BLOCK_DROPPED, preliminaryHash, "");
        MessageProperties properties = new MessageProperties();
        properties.setDeliveryMode(MessageProperties.DEFAULT_DELIVERY_MODE.PERSISTENT);
        Message message = jsonMessageConverter.toMessage(droppedTaskStatus, properties);
        rabbitTemplate.send(RabbitMQConfig.BLOCKCHAIN_EXCHANGE, "", message);
        logger.warn("MiningTaskNotifier: Notificado que el bloque: " + preliminaryHash + " fue descartado. Evento CANDIDATE_BLOCK_DROPPED publicado.");
    }
}